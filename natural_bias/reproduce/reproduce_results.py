...
if __name__ == "__main__":
    ...
    for i in range(1, 6):  # 5 ratios tested for hypothesis bias - [0.6, 0.7, 0.8, 0.9, 1.0]
        ...
        exp_dir = f'../models/hypothesis_bias/snli/early_stopping/exp3/ratio{i}'
        ratio = []
    ...
    for i in range(1, 5):
        # prepare all directories
        exp_dir = f'../models/overlap_bias/mnli/binary/early_stopping/exp3/ratio{i}'
        ratio = []
    ...
    for i in range(1, 5):  # 4 strengths tested for hypothesis bias - [(0.5, 0.7), (0.6, 0.8), (0.7, 0.9), (0.8, 1.0)]
        # Prepare all directories
        exp_dir = f'../models/hypothesis_bias/snli/early_stopping/exp5/prob{i}'
        probs = []
        erm_model_dirs, irm_model_dirs = read_dirs(exp_dir, dataset='SNLI')
    ...
    for i in range(1, 5):  # 4 strengths tested for hypothesis bias - [(0.5, 0.7), (0.6, 0.8), (0.7, 0.9), (0.8, 1.0)]
        # prepare all directories
        exp_dir = f'../models/overlap_bias/mnli/binary/early_stopping/exp5/prob{i}'
        probs = []
        erm_model_dirs, irm_model_dirs = read_dirs(exp_dir, dataset='MNLI_Binary')
    ...
    shutil.copytree('../models/hypothesis_bias/snli/early_stopping/exp1', '../models/hypothesis_bias/snli/early_stopping/exp4_fixed_val_size/size0', dirs_exist_ok=True)
    shutil.copytree('../models/overlap_bias/mnli/binary/early_stopping/exp1', '../models/overlap_bias/mnli/binary/early_stopping/exp4_fixed_val_size/size0', dirs_exist_ok=True)
    ...
    # for i, size_d in enumerate(size_dirs):
    #     # prepare all directories
    #     exp_dir = f'../models/overlap_bias/mnli/binary/early_stopping/exp4/' + size_d
    #     erm_model_dirs, irm_model_dirs = read_dirs(exp_dir, dataset='MNLI_Binary')
    #
    #     # <editor-fold desc="read data">
    #     # dict of dict - keys are subsets, subkeys are mean, std, min and max
    ...
    # for i, size_d in enumerate(size_dirs):
    #     # prepare all directories
    #     exp_dir = f'../models/hypothesis_bias/snli/early_stopping/exp4/' + size_d
    #     erm_model_dirs, irm_model_dirs = read_dirs(exp_dir, dataset='SNLI')
    #
    #     # <editor-fold desc="read data">
    #     # dict of dict - keys are subsets, subkeys are mean, std, min and max
    ...
    for i, size_d in enumerate(size_dirs):
        # prepare all directories
        exp_dir = f'../models/overlap_bias/mnli/binary/early_stopping/exp4_fixed_val_size/' + size_d
        erm_model_dirs, irm_model_dirs = read_dirs(exp_dir, dataset='MNLI_Binary')
    ...
    fig.savefig('overlap_bias_dataset_size_fixed_val_exp.png')
    ...
    for i, size_d in enumerate(size_dirs):
        # prepare all directories
        exp_dir = f'../models/hypothesis_bias/snli/early_stopping/exp4_fixed_val_size/' + size_d
        erm_model_dirs, irm_model_dirs = read_dirs(exp_dir, dataset='SNLI')
    ...
    fig.savefig('hypothesis_bias_dataset_size_fixed_val_exp.png')
    ...
    # # <editor-fold desc="Hypothesis bias - early stopping exp">
    # # exp4 - train ERM and IRM on p1=0.7 p2=0.9 with changing dataset size (also 5 seeds)
    ...
    # exp_dir_list = ['../models/hypothesis_bias/snli/exp1', '../models/hypothesis_bias/snli/early_stopping/exp1']
    # markers = ['x', 'o']
    # colors = {'run1': 'r', 'run2': 'g', 'run3': 'b', 'run4': 'y'}
    #
    # n = 4
    # for e, exp_dir in enumerate(exp_dir_list):
    #     # <editor-fold desc="prepare data">
    #
    #     # <editor-fold desc="load results to dictionaries">
    ...
    #     # </editor-fold>
    #
    #     # test ood - just 1 val for each run
    #     erm_acc_dict, _, _ = calc_mean_var_for_test(os.sep.join([exp_dir, 'erm']), 'test_splits', verbose=False)
    #     irm_acc_dict, _, _ = calc_mean_var_for_test(os.sep.join([exp_dir, 'irm']), 'test_splits', verbose=False)
    #
    #     erm_test_ood_acc = dict(
    #         [('run' + str(i), erm_acc_dict['bias unaligned split']['vals'][i - 1]) for i in range(1, n + 1)])
    #     irm_test_ood_acc = dict(
    #         [('run' + str(i), irm_acc_dict['bias unaligned split']['vals'][i - 1]) for i in range(1, n + 1)])
    #     # </editor-fold>
    #
    #     # <editor-fold desc="plot">
    #     # erm
    ...
    # # </editor-fold>
    #
    # # <editor-fold desc="Overlap bias - early stopping exp">
    # # exp4 - train ERM and IRM on p1=0.7 p2=0.9 with changing dataset size (also 5 seeds)
    # erm_fig, erm_axes = plt.subplots(nrows=4, ncols=3, figsize=(20.0, 16.0), dpi=240, sharex=True)
    # irm_fig, irm_axes = plt.subplots(nrows=4, ncols=3, figsize=(20.0, 16.0), dpi=240, sharex=True)
    # exp_dir_list = ['../models/overlap_bias/mnli/binary/exp1', '../models/overlap_bias/mnli/binary/early_stopping/exp1']
    # markers = ['x', 'o']
    ...
    #
    #     # test ood - just 1 val for each run
    #     erm_acc_dict, _, _ = calc_mean_var_for_test(os.sep.join([exp_dir, 'erm']), 'dev_mismatched_splits', verbose=False)
    #     irm_acc_dict, _, _ = calc_mean_var_for_test(os.sep.join([exp_dir, 'irm']), 'dev_mismatched_splits', verbose=False)
    #
    #     erm_test_ood_acc = dict(
    #         [('run' + str(i), erm_acc_dict['bias unaligned split']['vals'][i - 1]) for i in range(1, n + 1)])
    #     irm_test_ood_acc = dict(
    ...
    #     for axe, erm_subset_vals, metric in zip(erm_axes.tolist(), [[erm_train_acc, erm_val_acc, erm_val_ood_acc],
    #                                                                 [erm_train_error, erm_val_error, erm_val_ood_error],
    #                                                                 [erm_train_penalty, erm_val_penalty,
    #                                                                  erm_val_ood_penalty],
    #                                                                 [erm_train_loss, erm_val_loss, erm_val_ood_loss]],
    #                                             ['accuracy', 'error', 'penalty', 'loss']):
    #         for ax, erm_vals in zip(axe, erm_subset_vals):
    ...
    #     for axe, irm_subset_vals, metric in zip(irm_axes.tolist(), [[irm_train_acc, irm_val_acc, irm_val_ood_acc],
    #                                                                 [irm_train_error, irm_val_error, irm_val_ood_error],
    #                                                                 [irm_train_penalty, irm_val_penalty,
    #                                                                  irm_val_ood_penalty],
    #                                                                 [irm_train_loss, irm_val_loss, irm_val_ood_loss]],
    #                                             ['accuracy', 'error', 'penalty', 'loss']):
    #         for ax, irm_vals, subset in zip(axe, irm_subset_vals, ['train', 'val', 'val_ood']):
    ...
    # # </editor-fold>
    #
    # erm_fig.savefig('overlap_bias_early_stopping_exp_erm.png')
    # irm_fig.savefig('overlap_bias_early_stopping_exp_irm.png')
    # # </editor-fold>
